...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance vkInstance, VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int width, int height, uint32_t min_image_count)
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
    (void)vkInstance;
...
void ImGui_ImplVulkanH_DestroyWindow(VkInstance vkInstance, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator)
{
    ...
    vkDestroySurfaceKHR(vkInstance, wd->Surface, allocator);
